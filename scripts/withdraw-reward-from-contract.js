...
async function main() {
  ...
  console.log(`代币: ${DOGE_CONFIG.symbol}`)
  console.log(`代币地址: ${DOGE_CONFIG.tokenAddress}`)
  console.log(`池索引: ${DOGE_CONFIG.poolIndex}`)
  console.log('========================================\n')
  ...
  // 检查合约地址的代币余额
  const rewardToken = new ethers.Contract(DOGE_CONFIG.tokenAddress, ERC20_ABI, provider)
  ...
  console.log(`📊 余额信息:`)
  console.log(`   合约地址余额: ${formatUnits(contractBalance, DOGE_CONFIG.decimals)} ${DOGE_CONFIG.symbol}`)
  console.log(`   Owner 地址余额: ${formatUnits(ownerBalance, DOGE_CONFIG.decimals)} ${DOGE_CONFIG.symbol}\n`)
  ...
  // 询问是否提取全部余额
  console.log(`💡 准备提取: ${formatUnits(contractBalance, DOGE_CONFIG.decimals)} ${DOGE_CONFIG.symbol}`)
  console.log(`   这将把代币从合约地址转回 owner 地址\n`)

  try {
    console.log('📤 发送交易...')
    const tx = await stakingContract.emergencyWithdrawReward(DOGE_CONFIG.poolIndex, contractBalance)
    ...
    console.log(`📊 提取后余额:`)
    console.log(`   合约地址余额: ${formatUnits(newContractBalance, DOGE_CONFIG.decimals)} ${DOGE_CONFIG.symbol}`)
    console.log(`   Owner 地址余额: ${formatUnits(newOwnerBalance, DOGE_CONFIG.decimals)} ${DOGE_CONFIG.symbol}`)
  } catch (error) {
