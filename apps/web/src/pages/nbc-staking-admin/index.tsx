...
const NbcStakingAdmin: React.FC = () => {
  ...
  const handleFetchRequests = useCallback(async () => {
    ...
    try {
      ...
      if (count === 0) { setWithdrawRequests([]); return }
      ...
      const requests: WithdrawRequest[] = results
        .map((res, i) => {
          if (res.status !== 'success') return null
          const [poolIndex, user, amount, requestedAt, approved, executed, rejected] = res.result as any[]
          return {
            requestId: i,
        ...
        .filter(Boolean) as WithdrawRequest[]
      ...
      {activeTab === 'monitor' && (
        <Box p="24px" mb="24px">
          ...
          <Text fontSize="14px" color="textSubtle" mb="24px">
            {t('Review and approve or reject user withdrawal requests. Funds are locked until you approve.')}
          </Text>
              ...
              <Box>
                ...
                <Text fontSize="13px" color="textSubtle" mt="4px">
                  {isPaused
                    ...
                    : t('Contract is active. Users can stake and submit withdrawal requests.')}
                </Text>
              </Box>
              <Button
          ...
          {/* 统计 */}
          <Flex mb="16px" style={{ gap: '16px' }} flexWrap="wrap">
            <Box p="12px 20px" style={{ background: 'rgba(255,193,7,0.1)', borderRadius: '8px', border: '1px solid rgba(255,193,7,0.3)', minWidth: '100px', textAlign: 'center' }}>
              <Text fontSize="22px" bold color="warning">{withdrawRequests.filter(r => !r.approved && !r.rejected && !r.executed).length}</Text>
              <Text fontSize="12px" color="textSubtle">{t('Pending')}</Text>
            </Box>
            <Box p="12px 20px" style={{ background: 'rgba(49,208,170,0.1)', borderRadius: '8px', border: '1px solid rgba(49,208,170,0.3)', minWidth: '100px', textAlign: 'center' }}>
              <Text fontSize="22px" bold color="success">{withdrawRequests.filter(r => r.approved && !r.executed).length}</Text>
              <Text fontSize="12px" color="textSubtle">{t('Approved')}</Text>
            </Box>
            <Box p="12px 20px" style={{ background: 'rgba(255,77,77,0.1)', borderRadius: '8px', border: '1px solid rgba(255,77,77,0.3)', minWidth: '100px', textAlign: 'center' }}>
              <Text fontSize="22px" bold color="failure">{withdrawRequests.filter(r => r.rejected).length}</Text>
              <Text fontSize="12px" color="textSubtle">{t('Rejected')}</Text>
            </Box>
            <Box p="12px 20px" style={{ background: 'rgba(118,69,217,0.1)', borderRadius: '8px', border: '1px solid rgba(118,69,217,0.3)', minWidth: '100px', textAlign: 'center' }}>
              <Text fontSize="22px" bold>{withdrawRequests.filter(r => r.executed).length}</Text>
              <Text fontSize="12px" color="textSubtle">{t('Executed')}</Text>
            </Box>
          </Flex>

          {/* 申请列表 */}
          <Box p="16px" style={{ border: '1px solid rgba(118, 69, 217, 0.2)', borderRadius: '8px' }}>
            <Flex justifyContent="space-between" alignItems="center" mb="16px" flexWrap="wrap" style={{ gap: '8px' }}>
              <Text bold fontSize="16px">
                📋 {t('Requests')} ({withdrawRequests.length} {t('total')})
              </Text>
              <Flex style={{ gap: '8px' }} flexWrap="wrap">
                {(['pending', 'approved', 'rejected', 'all'] as const).map((f) => (
                  <Button
                    key={f}
                    scale="sm"
                    variant={monitorFilter === f ? 'primary' : 'tertiary'}
                    onClick={() => setMonitorFilter(f)}
                  >
                    {f === 'pending' ? `⏳ ${t('Pending')}` : f === 'approved' ? `✅ ${t('Approved')}` : f === 'rejected' ? `❌ ${t('Rejected')}` : t('All')}
                  </Button>
                ))}
                <Button
                  scale="sm"
            ...
            {withdrawRequests.length === 0 ? (
              <Box textAlign="center" py="32px">
                <Text fontSize="14px" color="textSubtle" mb="12px">{t('No requests loaded yet.')}</Text>
                        ...
                        <Flex justifyContent="space-between" alignItems="flex-start" flexWrap="wrap" style={{ gap: '8px' }}>
                          <Box style={{ flex: 1, minWidth: 0 }}>
                            <Flex alignItems="center" mb="4px" style={{ gap: '8px' }}>
                              <Text fontSize="12px" color="textSubtle">#{req.requestId}</Text>
                              <Text fontSize="12px" bold color={statusColor}>{statusLabel}</Text>
                            </Flex>
